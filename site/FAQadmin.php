<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>SmartBeing</title>
    <link rel="stylesheet" href="FAQadmin.css">
</head>
<body>
   <div class="bg">
        <form action="#" method="POST">
          <h2>BIENVENUE DANS LA FAQ</h2>
           <div class="line"></div>

          <h3>QUESTIONS COURANTES</h3>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
          <h4>Comment faire pour créer un compte ?</h4>

          <p>Pour créer votre compte, veuillez suivre les instructions de la FAQ pour la question "Comment faire pour créer un compte ?".</p>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
           <h4>Comment vous contacter ? </h4>
          <p>pour nous contacter vous pouvez utilisez la boite mail intégrer au site, pour cela appuyez simplement sur le bouton correspondant.</p>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
           <h4>Comment fonctionne les tests ?</h4>
          <p>Je ne sais pas.</p>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
           <h4>Comment récuperer mes scores ?</h4>
          <p>Pour récuperer vos scores veuillez allez dans l'onglet "score" de votre profil qui se trouve quelque part sur le site.</p>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
          <h4>Comment supprimer mon compte ?</h4>
          <p>Vous ne pouvez pas.</p>

          <button class="buttonDelete"><a></a></button>
          <button class="buttonModif"><a></a></button>
          <h4>Où doit on effectuer les tests ?</h4>
          <p>Dans une institut spécialisé comment un truc d'auto école</p>

          <button class="button"><a style="color:white; text-decoration:none" href="FAQ.html">Ajouter une question</a></button>
        </form>
</div>
</body>
</html>